...
`bin/generate_cds.py`, `bin/longest_isoform.py`, `bin/get_matching_cds.py` and `bin/refseq_longest_isoform.py` are standalone data-prep helpers, not called by any process. `longest_isoform.py` regexes a gene id out of the protein defline (Ensembl-style); **RefSeq protein deflines carry no gene id at all**, so `refseq_longest_isoform.py` drives selection from the CDS file instead, grouping on `[db_xref=GeneID:...]`, picking the longest CDS per gene, and emitting protein/CDS pairs whose record IDs already match. `generate_cds.py` reverse-translates a proteome into plausible CDS and is how the non-agalactiae test CDS files were made — those are synthetic, so dS values in the test profile are meaningless; the test is a smoke test, not a scientific fixture.
