...
- **Do not seed the branch lengths.** `write_tree` strips OrthoFinder's amino-acid branch lengths and runs `fix_blength = 0`, so CODEML starts each branch from its own initialisation. Seeding them instead (`fix_blength = 1`) looks like an obvious win — PAML re-estimates from a starting point instead of from scratch — and it is not. Measured on OG0000080 of the fish run (60 seqs, 1422 codons, 3 replicates, PAML 4.10.7): seeding from the **already-fitted optimum** gives 1.59×, which is the unobtainable upper bound; a uniform, uninformative seed gives 1.24×; and a seed 2× too long is **0.92×, slower than not seeding at all**. Since a cheap seed's whole problem is not knowing the scale, that last case is the expected one. Worse, the uninformative seed converged to a *different, inferior* optimum — lnL −27837.5325 against −27837.6439, with 10 of 117 branches' dS moved and one collapsing from 0.0022 to 0.0000. Zeroing a short branch corrupts exactly the recent duplications the pipeline exists to date, where the branch table's 4-decimal precision is already the floor. A 1.24× that can silently move the primary estimate is not a trade worth making. (Sampled `2NG.t` is written by CODEML but is evidently *not* used to initialise the ML branch lengths, which is why the headroom exists at all.)
