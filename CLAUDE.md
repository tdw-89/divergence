...
- **So paralog count is cheap and orthogroup size is not.** `C(100,2) = 4950` pairs is ~7 min; a 500-sequence tree fit is ~9 min at 300 codons, ~15 min at vertebrate CDS length. `params.codeml_timeout` bounds the tree fit but is no defence against pair count: each pairwise call is already sub-0.1 s, so only the count grows, and the count is cheap. Set `max_target_paralogs` generously.
- **`params.codeml_method` still defaults to `0`**, which is ~25× slower at n ≥ 32 for dS identical to 5 d.p. Flipping the default to `1` is a one-line change.
- **Re-indexing every CDS file on every `ks.py` call is not a bottleneck**: 176,000 records across 8 files (297 MB) parse in 1.9 s at 0.54 GB RSS.
...
- **`CODEML_BATCH` requests 6 CPUs (`process_medium`) but `ks.py` is strictly serial and PAML is single-threaded**, so five cores idle for the whole task. On a scheduler that bills reserved cores this multiplies the charged cost sixfold. Either set `cpus = 1` or parallelise the bash loop with `xargs -P ${task.cpus}` — the loop body is already independent per orthogroup and tolerates per-file failures. `MAFFT_BATCH` does pass `--thread ${task.cpus}`, so it is not affected.
...
- **`MAFFT_BATCH` has no per-file failure tolerance and no timeout**, unlike `CODEML_BATCH`: the loop runs under `bash -ue`, so one failed `mafft` fails the whole batch. `params.max_orthogroup_seqs` is applied in `extract_paralogs.py` before anything is written, which makes it the only guard on the MAFFT side.
